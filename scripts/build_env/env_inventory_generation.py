...
SCHEMAS_DIR = getenv("JSON_SCHEMAS_DIR", path.join(
    path.dirname(path.dirname(path.dirname(__file__))), "schemas"))
...
class Environment:
    ...
    def __post_init__(self):
        self.env_path = path.join(
            self.base_dir, "environments", self.cluster, self.name)
        ...
        self.inventory = helper.openYaml(
            self.inventory_path, allow_default=True)
        print(f"inventory: {self.inventory}")

        self.creds = helper.decrypt_file(
            self.creds_path, in_place=False, allow_default=True)
        print(f"creds: {self.creds}")

        self.inv_gen_creds = helper.decrypt_file(
            self.inv_gen_creds_path, in_place=False, allow_default=True)
...
def generate_env():
    ...
    logger.info(
        f"Starting env inventory generation for env: {env.name} in cluster: {env.cluster}")
...
def handle_sd(env, sd_source_type, sd_version, sd_data, sd_delta):
    ...
    if not sd_source_type:
        logger.info(
            "SD_SOURCE_TYPE is not specified, skipping SD file creation")
        return
    if sd_delta == "true":
        ...
        if not helper.check_file_exists(full_sd_path):
            logger.error(
                f"To use the delta processing mode for SD, a complete SD must already be stored in the repository.\nThe file {full_sd_path} does not exist.")
            exit(1)
    else:
    ...
    if sd_source_type == "artifact":
        ...
        logger.error(
            f'SD_SOURCE_TYPE must be set either to "artifact" or "json"')
        exit(1)
...
def extract_sd_from_json(env, sd_path, sd_data, sd_delta):
    if not sd_data:
        logger.error(
            f"SD_SOURCE_TYPE is set to 'json', but SD_DATA was not given in pipeline variables")
        exit(1)
    ...
    if sd_delta == "true":
        ...
        merge_sd(env, data)
    logger.info(
        f"SD successfully extracted from SD_DATA and is saved in {sd_path}")
...
def download_sd_with_version(env, sd_path, sd_version, sd_delta):
    ...
    if not sd_version:
        logger.error(
            f"SD_SOURCE_TYPE is set to 'artifact', but SD_VERSION was not given in pipeline variables")
        exit(1)
    ...
    r = ansible_runner.run(
        playbook='/module/ansible/download_sd_file.yaml', envvars=ansible_vars, verbosity=2)
    if (r.rc != 0):
        logger.error(
            f"Error during ansible execution. Result code is: {r.rc}. Status is: {r.status}")
        raise ReferenceError(
            f"Error during ansible execution. See logs above.")
    else:
        logger.info(
            f"Ansible execution status is: {r.status}. Stats is: {r.stats}")
...
def handle_env_inventory_init(env, env_inventory_init, env_template_version):
    if env_inventory_init != "true":
        logger.info(
            f"ENV_INVENTORY_INIT is not set to 'true'. Skipping env inventory initialization")
        return
    logger.info(
    ...
    helper.set_nested_yaml_attribute(
        env.inventory, 'inventory.environmentName', env.name)
    helper.set_nested_yaml_attribute(
        env.inventory, 'envTemplate.artifact', env_template_version)
    helper.set_nested_yaml_attribute(
        env.inventory, 'envTemplate.additionalTemplateVariables', helper.get_empty_yaml())
    helper.set_nested_yaml_attribute(
        env.inventory, 'envTemplate.envSpecificParamsets', helper.get_empty_yaml())


def handle_env_specific_params(env, env_specific_params):
    if not env_specific_params or env_specific_params == "":
        logger.info(
            f"ENV_SPECIFIC_PARAMS are not set. Skipping env inventory update")
        return
    ...
    helper.merge_yaml_into_target(
        env.inventory, 'envTemplate.additionalTemplateVariables', additionalTemplateVariables)
    helper.merge_yaml_into_target(
        env.inventory, 'envTemplate.envSpecificParamsets', envSpecificParamsets)
...
def handle_tenant_name(env, tenantName):
    if not tenantName:
        return
    helper.set_nested_yaml_attribute(
        env.inventory, 'inventory.tenantName', tenantName)
...
def handle_deployer(env, deployer):
    if not deployer:
        return
    helper.set_nested_yaml_attribute(
        env.inventory, 'inventory.deployer', deployer)
...
def handle_env_template_name(env, env_template_name):
    if not env_template_name:
        return
    helper.set_nested_yaml_attribute(
        env.inventory, 'envTemplate.name', env_template_name)
...
def handle_credentials(env, creds):
    ...
    sharedMasterCredentialFiles = helper.get_or_create_nested_yaml_attribute(
        env.inventory, 'envTemplate.sharedMasterCredentialFiles', default_value=[])
    ...
    helper.set_nested_yaml_attribute(
        env.inventory, 'envTemplate.sharedMasterCredentialFiles', sharedMasterCredentialFiles)
...
def handle_cluster_params(env, cluster_params):
    ...
    if 'clusterEndpoint' in cluster_params:
        helper.set_nested_yaml_attribute(
            env.inventory, 'inventory.clusterUrl', cluster_params['clusterEndpoint'])
    if 'clusterToken' in cluster_params and 'cloud-deploy-sa-token' not in env.creds:
        ...
        helper.set_nested_yaml_attribute(
            env.creds, 'cloud-deploy-sa-token', cred, is_overwriting=False)
