...
def find_env_instances_dir(env_name, instances_dir):
    ...
    if len(dirList) > 1:
        logger.error(
            f"Duplicate directories for {env_name} found in {instances_dir}: \n\t" + ",\n\t".join(str(x) for x in dirList))
...
def getenv_with_error(var_name):
    ...
    if not var:
        raise ValueError(
            f'Required value was not given and is not set in environment as {var_name}')
    return var
...
def check_environment_is_valid_or_fail(environment_name, cluster_name, instances_dir, skip_env_definition_check=False, validate_env_definition_by_schema=False, schemas_dir=""):
    env_dir = get_env_instances_dir(
        environment_name, cluster_name, instances_dir)
    ...
    if not check_dir_exists(env_dir):
        ...
        raise ReferenceError(
            f"Validation of environment folder '{env_dir}' failed. See logs above.")
    ...
    if skip_env_definition_check:
        ...
        logger.info(
            f"Environment {cluster_name}/{environment_name} validation is succesful")
        return
...
def check_env_definition_is_valid_or_fail(env_definition_path, schemas_dir):
    ...
    try:
        ...
        raise ValueError(
            f"Validation of env_definition in '{env_definition_path} failed. See logs above'") from None
...
def findResourcesBottomTop(sourceDir, stopParentDir, pattern, notPattern="", additionalRegexpPattern="", additionalRegexpNotPattern="", searchJsons=False):
    ...
    if parentPath not in sourcePath.parents:
        logger.error(
            f"Error while finding resources. {stopParentDirAbs} is not in parents of {sourceDirAbs}.")
        raise ReferenceError(
...
def __findResourcesBottomTop__(sourceDir, stopParentDir, pattern, notPattern, additionalRegexpPattern, additionalRegexpNotPattern, searchJsons, result, foundMap):
    ...
    findResults = findYamls(sourceDir, pattern, notPattern,
                            additionalRegexpPattern, additionalRegexpNotPattern)
    if searchJsons:
        findResults = merge_lists(findResults, findJsons(
            sourceDir, pattern, notPattern, additionalRegexpPattern, additionalRegexpNotPattern))
    for foundFile in findResults:
        ...
        if fileName not in foundMap:
            ...
            if len(findResults) == 1:
                ...
                raise ReferenceError(
                    f"Duplicate resource file with pattern {pattern} found. See logs above.")
...
def getEnvDefinition(env_dir):
    ...
    if not check_file_exists(envDefinitionPath):
        raise ReferenceError(
            f"Environment definition for env {env_dir} is not found in {envDefinitionPath}")
...
def getEnvCredentials(env_dir):
    ...
    if not check_file_exists(envCredentialsPath):
        logger.error(
            f"Credentials for env {env_dir} are not found in {envCredentialsPath}")
        raise ReferenceError(
            f"Credentials for env {env_dir} are not found. See logs above.")
...
def update_generated_versions(env_dir, stage_tag, template_version=""):
    ...
    if stage_tag == BUILD_ENV_TAG:
        ...
        store_value_to_yaml(
            versionsYaml, "generateEnvironmentLatestVersion", version, comment)
        ...
        store_value_to_yaml(
            versionsYaml, "cmdbImportLatestVersion", version, comment)
    else:
        version = "UNDEFINED"
    env_definition_path = getEnvDefinitionPath(env_dir)
    writeYamlToFile(env_definition_path, env_definition_yaml)
    logger.info(
        f"Generated version {version} updated for stage tag {stage_tag} in environment {env_definition_path}")
    ...
    if isinstance(data, dict):
        for key, value in data.items():
            if contains_cyrillic(key):
                logger.error(
                    f"Cyrillic character found in key '{key}' in file '{filename}'")
                error_found = True
            if isinstance(value, (dict, list)):
                ...
                logger.error(
                    f"Cyrillic character found in value '{value}' in file '{filename}' at path '{path}/{key}'")
                error_found = True
...
def find_cloud_passport_definition(env_instances_dir, instances_dir):
    ...
    if (cloud_passport_file_name):
        ...
        passportFilePath = findPassportInDefaultDirByName(
            cloudDir, cloudDirName)
        if passportFilePath:
...
def findPassportByEnvDefinition(env_instances_dir, instances_dir, cloud_passport_file_name):
    ...
    passportFiles = findResourcesBottomTop(
        env_instances_dir, instances_dir, f"/{cloud_passport_file_name}.y", "redentials/")
    if len(passportFiles) == 1:
        yamlPath = passportFiles[0]
        logger.info(
            f"Cloud passport file for {cloud_passport_file_name} found in: {yamlPath}")
        return yamlPath
        ...
        raise ReferenceError(
            f"Cloud passport with key {cloud_passport_file_name} not found in {instances_dir}")
...
def findPassportInDefaultDirByName(env_instances_dir, passport_name):
    ...
    passportFiles = findYamls(
        env_instances_dir, f"{DEFAULT_PASSPORT_DIR_NAME}/{passport_name}.y", notPattern="redentials/")
    if len(passportFiles) == 1:
        yamlPath = passportFiles[0]
        logger.info(
            f"Cloud passport file for {env_instances_dir} with name {passport_name} found in: {yamlPath}")
        return yamlPath
    elif len(passportFiles) > 1:
        logger.error(
        ...
        logger.info(
            f"Cloud passport for env {env_instances_dir} with name {passport_name} is not found. ")
        return ""
...
def find_cloud_name_from_passport(source_env_dir, all_instances_dir):
    ...
    if ("cloudPassport" in inventoryYaml["inventory"]):
        ...
        logger.info(
            f"Got cloud name from env_definition passport {cloudPassportFileName}")
    cloudPassportFile = find_cloud_passport_definition(
        source_env_dir, all_instances_dir)
    if cloudPassportFile:
        if f"{DEFAULT_PASSPORT_DIR_NAME}/{DEFAULT_PASSPORT_NAME}.y" in cloudPassportFile:
            ...
            logger.info(
                f"Got cloud name {cloudName} from environment directory {source_env_dir}")
            return cloudName
        else:
            cloudPassportFileName = extractNameFromFile(cloudPassportFile)
            logger.info(
                f"Cloud name '{cloudPassportFileName}' got from cloud passport file: {cloudPassportFile}")
