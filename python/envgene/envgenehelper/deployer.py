...
def get_cred_file_path(deployer_dir):
    ...
    if check_file_exists(dashes_cred_path):
        ...
        raise ReferenceError(
            f"Credentials definition for deployer not found. See logs above.")
    logger.info(
        f"Credentials definition for deployer is found in: {cred_path}")
    return cred_path
...
def get_value_and_attributes_from_cred(cred_macros, deployer_dir):
    ...
    if (check_is_envgen_cred(cred_macros)):
        ...
        cred_id, property = get_cred_id_and_property_from_cred_macros(
            cred_macros)
        ...
        if cred_id in data:
            ...
            logger.error(
                f"Credentials with key {cred_id} is not found in credentials file.")
            raise ReferenceError(
                f"Credentials with key {cred_id} is not found in credentials file.")
    else:
        return cred_macros
...
def get_deployer(env_name, instances_dir, failonerror=True):
    ...
    if not "deployer" in data["inventory"]:
        logger.error(
            f"Deployer definition for environment {env_path} is not specified in environment definition.")
        if failonerror:
            raise ReferenceError(
                f"Deployer definition for environment is not specified in environment definition. See logs above.")
        else:
            return ""
...
def find_deployer_definition(env_name, work_dir, instances_dir, failonerror=True):
    ...
    if len(deployers) == 1:
        ...
        logger.error(
            f"Duplicate deployer configuration found in {work_dir}: \n\t" + ",\n\t".join(str(x) for x in deployers))
        if failonerror:
            raise ReferenceError(
                f"Duplicate deployer configuration found in {work_dir} found. See logs above.")
            return ""
    else:
        if failonerror:
            raise ReferenceError(
                f"Deployer configuration not found in {work_dir}")
            return ""
...
def get_deployer_config(env_name, work_dir, instances_dir, secret_key=None, is_test=None, failonerror=True):
    ...
    deployer_file_path = find_deployer_definition(
        env_name, work_dir, instances_dir, failonerror)
    ...
    if deployer_name not in data:
        logger.info(
            f"Deployer with key {deployer_name} not found in: {deployer_file_path}. Going to root configuration: {basic_deployer_file_path}")
        deployer_dir = getDirName(basic_deployer_file_path)
        data = openYaml(basic_deployer_file_path)
    if deployer_name not in data:
        logger.error(
            f"Deployer with key {deployer_name} not found in either: {deployer_file_path} or {basic_deployer_file_path}")
        if failonerror:
            raise ReferenceError(
                f"Deployer with key {deployer_name} not found. See logs above.")
        else:
            return "", "", ""
    ...
    is_file_encrypted = is_encrypted(
        credentials_file_path, crypt_backend=envgene_config.get('crypt_backend'))
    cmdb_username, cmdb_username_attribute_path = get_value_and_attributes_from_cred(
        data[deployer_name]['username'], deployer_dir)
    cmdb_api_token, cmdb_api_token_attribute_path = get_value_and_attributes_from_cred(
        data[deployer_name]['token'], deployer_dir)
    if is_decryption_necessary and is_file_encrypted:
        if is_test:
            cred_yaml = decrypt_file(credentials_file_path, in_place=False,
                                     ignore_is_crypt=True, secret_key=secret_key, crypt_backend='Fernet')
        else:
            cred_yaml = decrypt_file(credentials_file_path, in_place=False)
        cmdb_username = get_or_create_nested_yaml_attribute(
            cred_yaml, cmdb_username_attribute_path)
        cmdb_api_token = get_or_create_nested_yaml_attribute(
            cred_yaml, cmdb_api_token_attribute_path)
