...
def readYaml(text, safe_load=False, context=None):
    ...
    if not resultYaml:
        logger.warning(
            f"Failed to read yaml. Returning empty dictionary. Context: {context}")
...
def alignYamlComments(yamlContent, extra_indent=0):
    ...
    if comments:
        max_col = max(
            map(lambda x: x[2].column if x[2] else 0, comments), default=0)
...
def get_nested_yaml_attribute_or_fail(yaml_content, attribute_str):
    ...
    for key in keys:
        if not isinstance(sub_content, dict):
            raise ValueError(
                f"Failed to find attribute '{attribute_str}', key '{key}' is not a dictionary")
        if key not in sub_content:
            raise ValueError(
                f"Failed to find attribute '{attribute_str}', key '{key}' doesn't exist")
...
def ensure_nested_attr_exists(yaml_content, attribute_str, default_value=None):
    sub_content, last_key = ensure_nested_attr_parents_exist(
        yaml_content, attribute_str)
...
def get_or_create_nested_yaml_attribute(yaml_content, attribute_str, default_value=None):
    yaml_content, key = ensure_nested_attr_exists(
...
def set_nested_yaml_attribute(yaml_content, attribute_str, value, comment="", is_overwriting=True):
    yaml_content, attribute = ensure_nested_attr_parents_exist(
        yaml_content, attribute_str)
    if attribute not in yaml_content:
        ...
        store_value_to_yaml(yaml_content, attribute, value, comment)
        logger.debug(
            f'Attribute {attribute_str} is changed to {value}. Old value was {old_value}')
    else:
        logger.debug(
            f'Setting {attribute_str} is skipped. Attribute already exists and is_overwriting is set to False')
...
def merge_yaml_into_target(yaml_content, target_attribute_str, source_yaml, overwrite_existing_values=True,  overwrite_existing_comments=True):
    ...
    if target_attribute_str != '':
        yaml_content, last_key = ensure_nested_attr_exists(
            yaml_content, target_attribute_str, get_empty_yaml())
    ...
    for k, v in source_yaml.items():
        ...
        if isinstance(target_yaml[k], dict) and isinstance(v, dict):
            merge_yaml_into_target(
                target_yaml[k], "", v, overwrite_existing_values, overwrite_existing_comments)
            ...
            for t_k, t_el in enumerate(target_yaml[k]):
                ...
                for t_el_k in t_el.keys():
                    if t_el_k in src_dicts[t_k].keys():
                        ...
                        break
                if merge:
                    target_yaml[k][t_k] = merge_yaml_into_target(
                        t_el, '', src_dicts[t_k], overwrite_existing_values, overwrite_existing_comments)
                    del src_dicts[k]
...
def merge_dict_key_with_comment(targetKey, targetYaml, sourceKey, sourceYaml, comment=""):
    if sourceKey in sourceYaml:
        ...
        store_value_to_yaml(targetYaml, targetKey,
                            sourceYaml[sourceKey], comment)
...
def log_jsonschema_validation_error(err):
    ...
    if "is not of type" in err.message:
        ...
        logger.error(
            f"Value of type {err.validator_value} is expected instead")
    elif "is a required property" in err.message:
        if len(key_path) > 0:
            err.message += f" at '{key_path}' property"
