...
async def _check_artifact_v2_async(app: Application, artifact_extension: FileExtension, version: str,
    ...
    if not env_creds:
        logger.debug(f"No env_creds for {app.name}, falling back to V1")
        return await _check_artifact_v1_async(app, artifact_extension, version)
    ...
    # MavenArtifact expects positional args: (group_id, artifact_id, version, version_by_spec, classifier='', extension='jar')
    maven_artifact = MavenArtifact(
        app.group_id,
        app.artifact_id,
        version,
        version,  # version_by_spec - use same as version
        '',  # classifier
        artifact_extension.value,  # extension
    )
