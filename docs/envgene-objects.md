...
- [EnvGene Objects](#envgene-objects)
  ...
  - [Instance Repository Objects](#instance-repository-objects)
    - [Environment Instance Objects](#environment-instance-objects)
      ...
      - [Shade Credentials File](#shade-credentials-file)
...
#### Shade Credentials File

Shade Credentials File is an encrypted YAML file containing exactly one Credential, enabling granular SOPS encryption to avoid full-file re-encryption on changes.

These files are auto-generated by a pre-commit hook and contain only a single Credential.
...
Location: Any YAML file in folder in folder whose name that starts with `shade`
...
Shade Credential Files store individual credentials in separate YAML files, allowing for more granular encryption and minimize unnecessary changes in version control history. They are auto-generated by EnvGene (or pre-commit hooks) from the main credential files when shade mode is enabled. When shade mode is enabled, for each credential in the original credential file, a separate shade file is created in the corresponding `shades-...` folder.

Each file contains a single credential object, conforming to [credential.schema.json](../../schemas/credential.schema.json). The file name is  `<Credential-ID>-cred.yml` The folder name is `shades-<original-credential-file-name>`.

See details in [Credential Encryption](/docs/features/cred-encryption.md#shade-credential-files)

Location:

- `/environments/<cluster-name>/<env-name>/Credentials/shades-*/*-cred.yml`
- `/environments/<cluster-name>/<env-name>/Inventory/credentials/shades-*/*-cred.yml`
- `/environments/<cluster-name>/credentials/shades-*/*-cred.yml`
- `/environments/credentials/shades-*/*-cred.yml`
- `/configuration/credentials/shades-*/*-cred.yml`
- `/environments/<cluster-name>/app-deployer/shades-*/*-cred.yml`
- `/environments/<cluster-name>/cloud-passport/shades-*/*-cred.yml`
